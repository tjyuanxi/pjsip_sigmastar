...
static void get_mixer_name(struct alsa_factory *af)
{
	 ...
	 elem = snd_mixer_elem_next(elem))
    {
        ...
        {
            if (snd_mixer_selem_has_playback_volume(elem) && strstr(elemname, "Playback Volume Lineout(-60~30)"))
            {
            ...
            if (snd_mixer_selem_has_capture_volume(elem) && strcmp(elemname, "Capture  Volume Amic") == 0)
            {
					      ...
					      pjmedia_aud_param *param)
{
    ...
    param->samples_per_frame = adi->default_samples_per_sec * 10 / 1000;
...
static int pb_thread_func (void *arg)
{
	...
	if (result == -EPIPE) {
	    PJ_LOG (3,(THIS_FILE, "pb_thread_func: underrun!"));
	    snd_pcm_prepare (pcm);
	} else if (result < 0) {
	    PJ_LOG (3,(THIS_FILE, "pb_thread_func: error writing data!"));
	}
			          ...
			          const pjmedia_aud_param *param)
{
    ...
    result = snd_pcm_hw_params_set_rate_near (stream->pb_pcm, params, &rate, NULL);
    TRACE_((THIS_FILE, "open_playback: clock rate set to: %d, %d, %s",rate, result, snd_strerror(result)));
    ...
    //			tmp_period_size : stream->pb_frames;					    					    
    TRACE_((THIS_FILE, "open_playback: period size set to: %d, %d,  %s",
	    tmp_period_size, result, snd_strerror(result)));
    ...
    TRACE_((THIS_FILE, "open_playback: buffer size set to: %d",
	    (int)tmp_buf_size));
    TRACE_((THIS_FILE, "open_playback: playback_latency set to: %d ms, %d, %s",
	    (int)stream->param.output_latency_ms, result, snd_strerror(result)));

    /* Activate the parameters */
    result = snd_pcm_hw_params (stream->pb_pcm, params);
    if (result < 0) {
	snd_pcm_close (stream->pb_pcm);
    PJ_LOG(4, (THIS_FILE, "=================:%s", snd_strerror(result)));
	return PJMEDIA_EAUD_SYSERR;
    }
				       ...
				       const void *value)
{
    ...
    printf("alsa_stream_set_cap:%d, %s, %s\n", cap, af->pb_mixer_name, af->cap_mixer_name);
	...
	if (cap == PJMEDIA_AUD_DEV_CAP_OUTPUT_VOLUME_SETTING) {
        ...
        printf("snd_mixer_selem_get_playback_volume_range:%d, %d\n", max, min);
		...
		return PJMEDIA_EAUD_SYSERR;
             ...
             printf("snd_mixer_selem_get_capture_volume_range:%d, %d\n", max, min);
