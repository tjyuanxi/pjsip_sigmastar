...
typedef struct vid4lin_stream
{
    ...
    pj_time_val			 pre_time;	/**< Time when started	*/
} vid4lin_stream;
...
static pj_status_t v4l2_scan_devs(vid4lin_factory *f)
{
	...
	for (j=0; fmt_cnt<PJ_ARRAY_SIZE(fmt_cap); ++j) {
        ...
        fdesc.pixelformat = V4L2_PIX_FMT_YUV420;
                                            ...
                                            pjmedia_frame *frame)
{
    ...
    // PJ_LOG(4,(THIS_FILE, "pjmedia_format_get_video_format_detail frame.size:%d", frame->size));
    // if (stream->io_type == IO_TYPE_MMAP)
    // {

        struct v4l2_buffer buf;
        pj_time_val time, tempTime;
        pj_gettimeofday(&time);
        pj_gettimeofday(&tempTime);
        PJ_TIME_VAL_SUB(tempTime, stream->pre_time);
        if ((tempTime.sec * 1000 + tempTime.msec) < (1000/DEFAULT_FPS)) {
            pj_thread_sleep(1000/DEFAULT_FPS - (tempTime.sec * 1000 + tempTime.msec));
        }
        PJ_TIME_VAL_SUB(time, stream->start_time);
        pj_gettimeofday(&stream->pre_time);
        frame->type = PJMEDIA_FRAME_TYPE_VIDEO;
        frame->bit_info = 0;
        frame->size = buf.bytesused;
        frame->timestamp.u64 = PJ_UINT64(1) * PJ_TIME_VAL_MSEC(time) *
        ...
        pjmedia_video_format_detail *vfd = pjmedia_format_get_video_format_detail(&stream->param.fmt, PJ_TRUE);
        if (vfd == NULL) {
            PJ_LOG(4,(THIS_FILE, "pjmedia_format_get_video_format_detail null"));
            return PJ_EINVALIDOP;
        }
        // PJ_LOG(4,(THIS_FILE, "pjmedia_format_get_video_format_detail w:%d,h:%d", vfd->size.w, vfd->size.h));
        frame->size = vfd->size.w * vfd->size.h * 5 / 4;

        pj_memset(frame->buf, 0x00, vfd->size.w * vfd->size.h);
        pj_memset(frame->buf + vfd->size.w * vfd->size.h, 0x80, vfd->size.w * vfd->size.h / 4);
        pj_memset(frame->buf + vfd->size.w * vfd->size.h * 5 / 4, 0x80, vfd->size.w * vfd->size.h / 4);
